...
tiny_stories_val_path = "data/data/TinyStoriesV2-GPT4-valid.txt"
tiny_stories_train_path = "data/data/TinyStoriesV2-GPT4-train.txt"
...
def pretokenize_text(text: str, special_tokens: list[str]) -> Counter[tuple[int, ...]]:
    ...
    for part in parts:
        ...
        # Normal text: apply GPT-2 style PAT regex
        for m in re.findall(PAT, part):
            if not m:
                continue
            bt = m.encode("utf-8")
            out[tuple(bt)] += 1
...
def count_pairs(
    ...
    for seq, c in inp.items():
        if c <= 0 or len(seq) < 2:
            continue

        # Count every occurrence (pair can repeat within a seq)
        for a, b in zip(seq, seq[1:]):
            p = (a, b)
            pairs_count[p] = pairs_count.get(p, 0) + c
...
def merge_pair(
    ...
    # Only sequences that (supposedly) contain old
    seqs = list(pairs_to_sequences.get(old, set()))
    for seq in seqs:
        cnt = byte_tokens_count.get(seq, 0)
        if cnt <= 0 or len(seq) < 2:
            continue
        ...
        if changed and tuple(merged) != seq:
            keys_to_remove.append(seq)
            keys_to_add.append((merged, cnt))
